...
public class MLDroneController : MonoBehaviour
{
    ...
    public Transform environmentParent; // Reference to the parent environment container

    [Header("Bounds (Local to Environment)")]
    public Vector2 xBounds = new Vector2(-50f, 50f); // Changed to local offsets
    public Vector2 zBounds = new Vector2(-50f, 50f); // Changed to local offsets
    ...
    void Awake()
    {
        ...
        // Auto-find environment parent if not assigned
    ...
    {
        // Generate local position within bounds
        ...
        // Convert to world position if there's a parent
        if (environmentParent != null)
        {
            transform.position = environmentParent.TransformPoint(localPos);
        }
        else
        {
    ...
    private Vector3 GetLocalPosition()
    {
        if (environmentParent != null)
        {
            return environmentParent.InverseTransformPoint(transform.position);
        }
